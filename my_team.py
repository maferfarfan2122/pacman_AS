...
class AgenteOfensivo(CaptureAgent):
    ...
    def choose_action(self, game_state):
        ...
        try:
            ...
            # Detect enemy ghosts (separate scared vs dangerous)
            enemigos = [game_state.get_agent_state(i) for i in self.get_opponents(game_state)]
            fantasmas_todos = [e for e in enemigos if not e.is_pacman and e.get_position() is not None]
            fantasmas = [f for f in fantasmas_todos if f.scared_timer <= 0]
            fantasmas_asustados = [f for f in fantasmas_todos if f.scared_timer > 0]
            
            # CHASE scared ghosts if close (bonus points)
            if fantasmas_asustados and not fantasmas:
                nearest_scared = min(fantasmas_asustados,
                    key=lambda f: self.get_maze_distance(mi_pos, f.get_position()))
                if self.get_maze_distance(mi_pos, nearest_scared.get_position()) <= 5:
                    return self.perseguir_objetivo(game_state, acciones, nearest_scared.get_position())
    ...
    def escapar(self, game_state, acciones, fantasmas):
        """Escape: prefer capsules, then flee toward home"""
        mi_pos = game_state.get_agent_state(self.index).get_position()
        capsulas = self.get_capsules(game_state)
        
        # If a capsule is reachable before ghosts catch us, go for it
        if capsulas:
            dist_ghost = min(self.get_maze_distance(mi_pos, f.get_position()) for f in fantasmas)
            nearest_cap = min(capsulas, key=lambda c: self.get_maze_distance(mi_pos, c))
            dist_cap = self.get_maze_distance(mi_pos, nearest_cap)
            if dist_cap < dist_ghost:
                return self.perseguir_objetivo(game_state, acciones, nearest_cap)
        
        # Otherwise flee: maximize ghost distance while favoring home direction
        walls = game_state.get_walls()
        frontera = [(self.frontera_x, y) for y in range(walls.height) if not walls[self.frontera_x][y]]
        ...
        for accion in acciones:
            ...
            # Bonus for moving toward home (avoid dead ends)
            home_bonus = 0
            if frontera:
                dist_home = min(self.get_maze_distance(pos_sig, p) for p in frontera)
                home_bonus = -0.5 * dist_home
            
            score = dist_min + home_bonus
    ...
    def perseguir_objetivo(self, game_state, acciones, target_pos):
        ...
        for accion in acciones:
            sucesor = self.get_successor(game_state, accion)
            pos_sig = sucesor.get_agent_state(self.index).get_position()
            dist = self.get_maze_distance(pos_sig, target_pos)
            if dist < mejor_dist:
                mejor_dist = dist
                mejor_accion = accion
...
class AgenteDefensivo(CaptureAgent):
    ...
    def choose_action(self, game_state):
        ...
        try:
            ...
            if mi_estado.scared_timer > 0 and invasores:
                return self.evitar_invasores(game_state, acciones, invasores)
    ...
    def evitar_invasores(self, game_state, acciones, invasores):
        """When scared, keep distance from invaders to avoid being eaten"""
        mejor_accion = None
        mejor_dist = -1
        ...
        for accion in acciones:
            ...
            dist_min = min(self.get_maze_distance(pos_sig, i.get_position()) for i in invasores)
